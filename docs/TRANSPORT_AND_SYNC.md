...
- **Доставка изменений от другого участника общего списка (`collaborationInbox`) не push-based.** `pollApplyAndAcknowledge()` вызывается только при старте приложения, при возврате `AppState` в `active` и сразу после собственной локальной мутации (что не помогает для *входящих* изменений). Реального push/wake-механизма нет, хотя он упомянут в кросс-репозиторном дизайн-спеке (§5, "Android wake hint") — эта часть спека не реализована. Пока получатель не откроет/развернёт yuNote заново, чужое изменение просто лежит в очереди на сервере. Найдено и задокументировано 2026-09-15; см. `IoT-Key-Fob-Project/ROADMAP.md` за статусом отдельной задачи на реальный push.
